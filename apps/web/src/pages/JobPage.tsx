...
import { jobDisplayTitle, jobPaperIdentifier } from "../lib/paper-job";
...
export function JobPage({ adminMode = false }: { adminMode?: boolean }) {
  ...
  const baseTitle = jobDisplayTitle(job, text("论文调研任务", "Literature research job"));
  const title = `${baseTitle}${job.file_names && job.file_names.length > 1 ? ` +${job.file_names.length - 1}` : ""}`;
  const identifier = jobPaperIdentifier(job);
