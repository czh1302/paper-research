...
import type { ReportRecord, ReportSectionName, SharedExperimentSummary } from "../lib/types";
import { mergeReportSection, SharedReportView } from "./ReportPage";

export function SharedReportPage() {
  const { text, formatDate } = useLanguage();
  const { token = "" } = useParams(); const [record, setRecord] = useState<ReportRecord | null>(null); const [experiments, setExperiments] = useState<SharedExperimentSummary[]>([]); const [expires, setExpires] = useState(""); const [error, setError] = useState("");
  const loadedSections = useRef(new Set<ReportSectionName>(["overview"]));
  useEffect(() => { void getSharedReport(token).then((data) => { setRecord(data.report); setExperiments(data.experiments); setExpires(data.expiresAt); }).catch((cause) => setError(cause instanceof Error ? cause.message : text("分享链接无效", "Invalid share link"))); }, [text, token]);
