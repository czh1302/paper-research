...
export function JobPage({ readOnly = false }: { readOnly?: boolean }) {
  ...
  useEffect(() => {
    ...
    async function load() { try { const next = await getJob(id, readOnly); setJob(next); const result = readOnly ? await client.from("job_events").select("*").eq("job_id", id).order("created_at") : await client.from("job_events").select("id,kind,data,created_at").eq("job_id", id).in("kind", [...publicEventKinds]).order("created_at"); setEvents(((result.data ?? []) as unknown as Partial<JobEvent>[]).map(safeEvent)); if (readOnly) { const attemptResult = await client.from("job_attempts").select("id,attempt_number,failure_category,checkpoint_stage,safe_error,created_at").eq("job_id", id).order("created_at", { ascending: false }); setAttempts((attemptResult.data ?? []) as unknown as JobAttempt[]); } if (next.status === "completed") setReport(await getReportByJob(id)); } catch (cause) { setError(cause instanceof Error ? cause.message : text("任务加载失败", "Could not load the job")); } }
    void load();
    const channel = client.channel(`job:${id}`).on("postgres_changes", { event: "UPDATE", schema: "public", table: "jobs", filter: `id=eq.${id}` }, (payload) => { const next = payload.new as JobRecord; setJob((current) => ({ ...next, error: readOnly ? next.error : null, file_names: current?.file_names })); if (next.status === "completed") void getReportByJob(id).then(setReport); }).on("postgres_changes", { event: "INSERT", schema: "public", table: "job_events", filter: `job_id=eq.${id}` }, (payload) => { const next = payload.new as JobEvent; if (readOnly || publicEventKinds.has(next.kind)) setEvents((current) => [...current, safeEvent(next)]); }).subscribe();
  ...
  return <div className="mx-auto max-w-4xl">
    ...
    {readOnly && job.error && <div className="mt-5 flex gap-3 rounded-xl border border-warning/25 bg-warning/[.07] p-4 text-warning"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0"/><span>{job.error}</span></div>}
