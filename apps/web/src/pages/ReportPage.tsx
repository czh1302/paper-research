...
function jointClaimText(item: string | { claim_zh: string; claim_en: string }, language: "zh" | "en") {
  return typeof item === "string" ? item : item[`claim_${language}`];
}
...
function ElementDetails({ title, values, language, evidenceMap, paperTitles }: { title: string; values: ProblemElement[]; language: "zh" | "en"; evidenceMap: Map<string, Evidence>; paperTitles: Map<string, string> }) {
  ...
  return <div><h4 className="report-label">{title}</h4><div className="mt-3 space-y-2">{values.map((item, index) => <details className="report-detail" key={`${item.name}-${index}`}><summary><span>{item.name}</span>{item.symbol && <code>{item.symbol}</code>}<ChevronRight className="h-4 w-4" /></summary><div className="report-copy report-copy-wide px-4 pb-4">{localized(item, "description", language)}{item.domain && <p className="report-meta mt-2">{item.domain}</p>}<div className="mt-3"><EvidenceCitations ids={item.evidence_ids} evidenceMap={evidenceMap} paperTitles={paperTitles}/></div></div></details>)}</div></div>;
}
...
function FlowColumn({ label, values, language, evidenceMap, paperTitles }: { label: string; values: ProblemElement[]; language: "zh" | "en"; evidenceMap: Map<string, Evidence>; paperTitles: Map<string, string> }) {
  return <div className="report-flow-column"><span className="report-label uppercase tracking-[.08em]">{label}</span><div className="mt-3 space-y-2">{values.slice(0, 4).map((item) => <details key={item.name} className="report-flow-item"><summary>{item.name}<ChevronRight className="h-3.5 w-3.5" /></summary><div className="report-copy report-copy-wide mt-2">{localized(item, "description", language)}<div className="mt-2"><EvidenceCitations ids={item.evidence_ids} evidenceMap={evidenceMap} paperTitles={paperTitles}/></div></div></details>)}{values.length === 0 && <span className="report-meta">—</span>}</div></div>;
}
...
function ProblemPanel({ problem, evidenceMap, paperTitles }: { problem: ProblemStatement; evidenceMap: Map<string, Evidence>; paperTitles: Map<string, string> }) {
  ...
  return <article className="panel p-5 sm:p-7">
    ...
    <div className="mt-5 rounded-xl bg-subtle/60 p-4 sm:p-5"><span className="report-label">{text("研究任务", "Research task")}</span><p className="report-copy mt-2">{localized(problem, "task", language)}</p><div className="mt-3"><EvidenceCitations ids={problem.task_evidence_ids} evidenceMap={evidenceMap} paperTitles={paperTitles}/></div></div>
    <div className="report-flow mt-5">
      ...
      <div className="report-flow-column report-flow-method"><span className="report-label uppercase tracking-[.08em]">{text("核心方法", "Core method")}</span><p className="report-copy report-copy-wide mt-3">{localized(problem, "algorithm", language)}</p><div className="mt-3"><EvidenceCitations ids={problem.algorithm_evidence_ids} evidenceMap={evidenceMap} paperTitles={paperTitles}/></div></div>
      <div className="report-flow-arrow">→</div>
...
function ThemeCard({ theme, papers }: { theme: ResearchTheme; papers: CandidatePaper[] }) {
  ...
  return <article className="panel p-5"><h3 className="!mt-0 !text-lg !text-content">{localized(theme, "title", language)}</h3><p className="report-copy mt-2">{localized(theme, "summary", language)}</p><div className="mt-4 space-y-3">{papers.map((paper) => <div className="rounded-xl border border-line p-3" key={paper.canonical_id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-sm font-semibold leading-5 text-content">{paper.title}</p><p className="report-meta mt-1">{[paper.year, paper.venue].filter(Boolean).join(" · ") || "—"}</p></div><SourceCitation url={paper.url} papers={[paper]}/></div></div>)}</div></article>;
}

function Score({ label, value, reason, uncertainty = false }: { label: string; value: number; reason: string; uncertainty?: boolean }) {
  const { language } = useLanguage();
  return <div className="rounded-xl bg-subtle/65 p-3"><div className="flex items-center justify-between gap-3"><span className="report-meta">{label}</span><strong className="text-sm text-content">{scoreLevel(value, language, uncertainty)}</strong></div>{reason && <p className="report-copy report-copy-wide mt-2">{reason}</p>}</div>;
}
...
function IdeaCard({ idea, papers }: { idea: PresentationIdea; papers: CandidatePaper[] }) {
  ...
  return <article className={`panel p-5 sm:p-6 ${idea.priority === 1 ? "report-recommended" : ""}`}>
    ...
    <p className="report-copy mt-4 font-medium">{localized(idea, "idea", language)}</p>
    {localized(idea, "recommendation_reason", language) && <p className="report-copy mt-3 rounded-lg bg-info/8 p-3">{localized(idea, "recommendation_reason", language)}</p>}
    <div className="mt-5 space-y-4">{fields.map(([label, value]) => <div key={label}><h4 className="report-label uppercase tracking-[.06em]">{label}</h4><p className="report-copy mt-1.5">{value}</p></div>)}</div>
...
function PapersDrawer({ open, onClose, papers }: { open: boolean; onClose: () => void; papers: CandidatePaper[] }) {
  ...
  return <div className="report-drawer-layer" role="presentation"><button className="report-drawer-backdrop" aria-label={text("关闭论文列表", "Close paper list")} onClick={onClose}/><aside className="report-drawer" role="dialog" aria-modal="true" aria-labelledby="all-papers-title"><div className="flex items-start justify-between gap-4 border-b border-line p-5"><div><h2 id="all-papers-title" className="!m-0 !text-xl">{text("全部检索结果", "All retrieval results")}</h2><p className="report-meta mt-1">{text(`${filtered.length} 篇去重候选`, `${filtered.length} deduplicated candidates`)}</p></div><button className="button button-secondary !h-9 !min-h-9 !w-9 !p-0" onClick={onClose} aria-label={text("关闭", "Close")}><X className="h-4 w-4"/></button></div><div className="grid gap-3 border-b border-line p-5 sm:grid-cols-[1fr_auto]"><label className="relative"><Search className="absolute left-3 top-3.5 h-4 w-4 text-faint"/><input autoFocus className="input !pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={text("搜索标题、作者、会议或来源", "Search title, author, venue, or source")}/></label><select className="input sm:w-40" value={sort} onChange={(event) => setSort(event.target.value as "relevance" | "year")}><option value="relevance">{text("相关性排序", "By relevance")}</option><option value="year">{text("年份排序", "By year")}</option></select></div><div className="flex-1 overflow-y-auto p-5"><div className="space-y-3">{visible.map((paper) => <article className="rounded-xl border border-line p-4" key={paper.canonical_id}><div className="flex items-start justify-between gap-3"><div><h3 className="!m-0 !text-base !text-content">{paper.title}</h3><p className="report-meta mt-2">{[paper.year, paper.venue, ...(paper.authors ?? []).slice(0, 3)].filter(Boolean).join(" · ")}</p></div><SourceCitation url={paper.url} papers={[paper]}/></div>{paper.abstract && <p className="report-copy report-copy-wide mt-3 line-clamp-3">{paper.abstract}</p>}</article>)}</div>{!visible.length && <p className="report-copy report-copy-wide py-12 text-center">{text("没有匹配论文", "No matching papers")}</p>}</div><div className="report-meta flex items-center justify-between border-t border-line p-4"><span>{page + 1} / {Math.max(1, Math.ceil(filtered.length / pageSize))}</span><div className="flex gap-2"><button className="button button-secondary !min-h-9 !py-1" disabled={page === 0} onClick={() => setPage((value) => Math.max(0, value - 1))}>{text("上一页", "Previous")}</button><button className="button button-secondary !min-h-9 !py-1" disabled={(page + 1) * pageSize >= filtered.length} onClick={() => setPage((value) => value + 1)}>{text("下一页", "Next")}</button></div></div></aside></div>;
}
...
function LegacyReportView({ record, shared = false }: { record: ReportRecord; shared?: boolean }) {
  ...
  return <article className="report-shell mx-auto max-w-6xl">
    ...
    <section className={`report-section mt-7 ${tab === "overview" ? "block" : "hidden"}`} role="tabpanel">
      <div className="report-hero"><span className="report-rank">{text("一句话结论", "Bottom line")}</span><h2 className="!mt-3 !text-2xl sm:!text-3xl">{localized(presentation, "headline", language)}</h2><p className="report-copy mt-4">{localized(presentation, "executive_summary", language)}</p></div>
