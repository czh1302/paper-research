...
function BriefOverview({ brief, evidenceMap, paperTitles }: { brief: ProblemBrief; evidenceMap: Map<string, Evidence>; paperTitles: Map<string, string> }) {
  ...
  return <div className="mt-6 grid gap-4 md:grid-cols-2">
    {cards.map((card) => <article className={`panel v4-brief-card ${card.className} min-w-0 p-5`} key={card.title}><h3 className="!m-0 !text-base !text-content">{card.title}</h3><div className="mt-4 min-w-0 space-y-3">{card.items.slice(0, 4).map((item, index) => <div className="min-w-0" key={`${item.label_en}-${index}`}><strong className="text-sm text-content">{localized(item, "label", language)}</strong><p className="mt-1 text-sm leading-6 text-muted">{localized(item, "explanation", language)}</p><div className="mt-2 min-w-0"><EvidenceCitations ids={item.evidence_ids} evidenceMap={evidenceMap} paperTitles={paperTitles}/></div></div>)}</div></article>)}
    <article className="panel v4-brief-card v4-brief-method min-w-0 p-5"><h3 className="!m-0 !text-base !text-content">{text("算法", "Algorithm")}</h3><ol className="mt-4 min-w-0 space-y-3">{brief.algorithm_steps.slice(0, 4).map((step) => <li className="grid min-w-0 grid-cols-[1.7rem_minmax(0,1fr)] gap-2 text-sm leading-6" key={step.order}><span className="v4-step-number">{step.order}</span><span className="min-w-0"><strong className="block text-content">{localized(step, "title", language)}</strong><span className="text-muted">{localized(step, "explanation", language)}</span><span className="mt-2 block min-w-0"><EvidenceCitations ids={step.evidence_ids} evidenceMap={evidenceMap} paperTitles={paperTitles}/></span></span></li>)}</ol></article>
