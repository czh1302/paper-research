...
def _uses_deterministic_exploratory_fallback(checkpoint: dict[str, Any]) -> bool:
    return any(
        isinstance(attempt, dict)
        and attempt.get("source") == "deterministic_exploratory_fallback"
        for attempt in checkpoint.get("pilot_compilation_attempts") or []
    )


def _deterministic_exploratory_repository(
    idea: dict[str, Any], specification: PilotSpecification
) -> tuple[RepositoryManifest, list[GeneratedRepositoryFile]]:
    """Build the executable proxy repository without another provider call.

    This path is deliberately limited to a PilotSpecification that already
    fell back to the deterministic exploratory contract above. It guarantees
    that a provider timeout cannot leave the user with an empty workspace.
    """

    title = str(
        idea.get("title_zh")
        or idea.get("title_en")
        or idea.get("title")
        or "Research Atlas exploratory pilot"
    ).strip()[:300]
    hypothesis_zh = specification.hypothesis_zh.strip()
    hypothesis_en = specification.hypothesis_en.strip()
    readme = f"""# {title}

这是 Research Atlas 为资源受限场景生成的可执行探索性代理仓库。
...
This repository validates the execution path, deterministic intervention signal,
tests, and frozen evaluator contract. It does **not** claim to validate the full
paper-level hypothesis. Replace the proxy data path with the declared real data
and baselines before making a scientific claim.
...
Generated artifacts are written under `artifacts/` and evaluated only by the
frozen Research Atlas evaluator.
"""
    proxy_source = '''from __future__ import annotations

import random


def fixed_inputs(seed: int = 17, size: int = 256) -> list[float]:
    """Return a reproducible, bounded proxy workload."""
    generator = random.Random(seed)
    return [generator.random() for _ in range(size)]


def baseline_score(values: list[float]) -> float:
    if not values:
        raise ValueError("values must not be empty")
    return sum(values) / len(values)


def intervention_score(values: list[float]) -> float:
    """Apply a fixed intervention without changing the input sample."""
    return sum(min(1.0, value + 0.05) for value in values) / len(values)


def run_proxy(seed: int = 17, size: int = 256) -> dict[str, float]:
    values = fixed_inputs(seed=seed, size=size)
    return {
        "baseline": baseline_score(values),
        "intervention": intervention_score(values),
    }
'''
...
from research_atlas_pilot.proxy import run_proxy


def main() -> None:
    raw = run_proxy()
    output = ROOT / "artifacts/raw_results.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps({"baseline": raw["baseline"]}, sort_keys=True), encoding="utf-8")
    print(json.dumps({"baseline": raw["baseline"]}, sort_keys=True))


if __name__ == "__main__":
    main()
'''
    intervention_source = '''from __future__ import annotations
...
if __name__ == "__main__":
    main()
'''
    test_source = '''from research_atlas_pilot.proxy import fixed_inputs, run_proxy


def test_proxy_is_reproducible() -> None:
    assert run_proxy() == run_proxy()


def test_intervention_preserves_inputs_and_has_a_signal() -> None:
    values = fixed_inputs()
    result = run_proxy()
    assert len(values) == 256
    assert result["intervention"] - result["baseline"] > 0.01
'''
    files_by_path = {
        "README.md": readme,
...
[project]
name = "research-atlas-exploratory-pilot"
version = "0.1.0"
...
testpaths = ["tests"]
''',
        "src/research_atlas_pilot/__init__.py": (
            '"""Deterministic exploratory pilot generated by Research Atlas."""\n'
        ),
        "src/research_atlas_pilot/proxy.py": proxy_source,
        "scripts/run_baseline.py": baseline_source,
        "scripts/run_intervention.py": intervention_source,
        "tests/test_proxy.py": test_source,
        "docs/EXPERIMENT.md": """# Experiment contract

The editable repository produces raw baseline and intervention observations.
The immutable evaluator under `.research-atlas/evaluator/` computes the final
metric and applies the frozen threshold. This separation prevents repository
code from rewriting the scientific success criterion.
""",
    }
    plan = [
        ("README.md", "Documents scope, limitations, and commands", "Markdown", 1),
        (".gitignore", "Excludes generated runtime files", "Git ignore", 1),
        ("pyproject.toml", "Defines the Python project and test discovery", "TOML", 1),
        (
            "src/research_atlas_pilot/__init__.py",
            "Defines the reusable pilot package",
            "Python",
            1,
        ),
        (
            "src/research_atlas_pilot/proxy.py",
            "Implements deterministic baseline and intervention logic",
            "Python",
            1,
        ),
        (
            "scripts/run_baseline.py",
            "Runs the frozen-input baseline and writes raw observations",
            "Python",
            1,
        ),
        (
            "scripts/run_intervention.py",
            "Runs the intervention and completes raw observations",
            "Python",
            1,
        ),
        ("tests/test_proxy.py", "Tests reproducibility and intervention signal", "Python", 1),
        (
            "docs/EXPERIMENT.md",
            "Explains separation between editable code and frozen evaluation",
            "Markdown",
            2,
        ),
    ]
    manifest = RepositoryManifest.model_validate(
        {
            "architecture_zh": "分层 Python 探索性代理：可复用机制模块、独立运行脚本、自动测试与冻结评价器分离。",
            "architecture_en": "A layered Python exploratory proxy with a reusable mechanism module, separate runners, automated tests, and an isolated frozen evaluator.",
            "files": [
                {
                    "path": path,
                    "purpose": purpose,
                ...
                for path, purpose, language, batch in plan
            ],
        }
    )
    files = [
        GeneratedRepositoryFile(path=item.path, content=files_by_path[item.path])
        ...
        try:
            ...
            if checkpoint.get("interpretation"):
                ...
                interpretation = ExperimentInterpretation(
                    summary_zh=(
                        "探索性代理的冻结评价器达到预设执行阈值；这只证明代码、测试和评价链路可重复运行，不等同于验证完整研究假设。"
                        if evaluation.passed
                        else "探索性代理的冻结评价器未达到预设执行阈值；该结果不等同于否定完整研究假设。"
                    ),
                    summary_en=(
                        "The frozen exploratory evaluator met its execution threshold. This demonstrates only a reproducible code, test, and evaluation path, not validation of the full research hypothesis."
                        if evaluation.passed
                        else "The frozen exploratory evaluator did not meet its execution threshold. This does not by itself reject the full research hypothesis."
                    ),
                )
            else:
                try:
