...
import { automaticExperimentEnabled, experimentPermissions, experimentReadiness, isUuid, manualExperimentEnabled, publicExperiment } from "../_shared/experiments.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [{ data: report, error }, { data: adminRow, error: adminError }] = await Promise.all([
      admin.from("reports").select("id,generation_id,job:jobs!inner(user_id)").eq("id", reportId).maybeSingle(),
    ...
    const { data, error: experimentsError } = await admin.from("idea_experiments")
      .select("*,experiment_runs!experiment_runs_experiment_id_fkey(count),experiment_runtime(state)").eq("report_id", reportId).is("deletion_requested_at", null)
      .eq("report_generation_id", report.generation_id)
    ...
    return json(request, {
      ...
      readiness: experimentReadiness(row, Array.isArray(row.experiment_runtime)
        ? row.experiment_runtime[0] ?? null : row.experiment_runtime ?? null),
