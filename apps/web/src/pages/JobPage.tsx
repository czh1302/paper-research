...
export function JobPage({ readOnly = false }: { readOnly?: boolean }) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {report && <Link className="panel mt-6 flex items-center justify-between border-cyan/30 p-6 transition hover:bg-cyan/[.05]" to={readOnly ? `/admin/reports/${report.id}` : `/reports/${report.id}`}><div><p className="eyebrow">Report ready</p><h2 className="mt-2 text-xl font-semibold text-paper">查看研究图谱与完整报告</h2></div><ArrowRight className="h-6 w-6 text-cyan" /></Link>}
