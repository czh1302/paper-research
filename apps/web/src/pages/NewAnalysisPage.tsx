  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <section className="panel mt-8 p-6 md:p-8">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2"><label><span className="label">最大循环轮数 / Maximum rounds</span><select className="input" value={rounds} onChange={(e) => setRounds(Number(e.target.value))}>{[1,2,3,4,5].map((n) => <option key={n} value={n}>{n} {n === 1 ? "轮（默认）" : "轮"}</option>)}</select><span className="mt-2 block text-xs text-slate-500">检索覆盖率收敛后可能提前停止。</span></label><div className="rounded-xl border border-white/10 bg-black/10 p-4"><div className="text-sm text-slate-400">预计处理量</div><strong className="mt-2 block text-lg text-paper">{files.length} PDF × {rounds} 轮</strong><div className="mt-2 text-xs text-cyan">内测期间不设置分析单元配额</div></div></div>
