    ...
    <>
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div>
          ...
          <h1 className="mt-3 flex items-center gap-3 text-4xl font-semibold tracking-tight text-content"><ShieldCheck className="h-9 w-9 text-warning" />全站管理</h1>
          <p className="mt-3 text-muted">查看全部用户和任务状态。此页面不允许修改或删除其他用户的数据。</p>
        </div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        <div className="panel p-5"><span className="grid h-10 w-10 place-items-center rounded-xl bg-accent/10"><Users className="h-5 w-5 text-accent-strong" /></span><div className="mt-4 text-3xl font-semibold text-content">{totalUsers}</div><div className="mt-1 text-sm text-muted">注册用户</div></div>
        <div className="panel p-5"><span className="grid h-10 w-10 place-items-center rounded-xl bg-warning/10"><ShieldCheck className="h-5 w-5 text-warning" /></span><div className="mt-4 text-3xl font-semibold text-content">{totalJobs}</div><div className="mt-1 text-sm text-muted">全站任务</div></div>
        <div className="panel p-5"><div className="font-mono text-xs font-medium text-accent-strong">ACCESS</div><div className="mt-4 text-lg font-semibold text-content">READ ONLY</div><div className="mt-1 text-sm text-muted">管理员跨用户只读审计</div></div>
      </div>

      <div className="mt-6">
        <label className="label" htmlFor="admin-search">筛选当前页</label>
        <input id="admin-search" className="input mt-2" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="邮箱、用户 ID、任务 ID 或 PDF 文件名" />
      </div>

      {error && <div className="mt-6 rounded-xl border border-danger/25 bg-danger/[.07] p-4 text-danger">{error}</div>}

      <section className="panel mt-8 overflow-hidden">
        <div className="flex flex-col justify-between gap-3 border-b border-line px-6 py-5 sm:flex-row sm:items-center"><h2 className="font-semibold text-content">全部用户</h2><Pager offset={userOffset} total={totalUsers} onChange={setUserOffset}/></div>
        <div className="overflow-x-auto">
          <table className="report-table min-w-[960px]">
            ...
            <tbody>{visibleUsers.map((user) => <tr key={user.user_id}>
              ...
              <td><div>{new Date(user.created_at).toLocaleString()}</div><div className="mt-1 text-xs text-muted">最近：{user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleString() : "尚未登录"}</div></td>
              <td><span className="text-content">{user.job_count}</span><span className="ml-2 text-xs text-muted">活跃 {user.active_job_count} · 完成 {user.completed_job_count}</span></td>
            </tr>)}</tbody>
          </table>
          {!loading && visibleUsers.length === 0 && <div className="p-10 text-center text-sm text-muted">当前页没有匹配用户</div>}
        </div>
      </section>

      <section className="panel mt-8 overflow-hidden">
        <div className="flex flex-col justify-between gap-3 border-b border-line px-6 py-5 sm:flex-row sm:items-center"><h2 className="font-semibold text-content">全部任务</h2><Pager offset={jobOffset} total={totalJobs} onChange={setJobOffset}/></div>
        <div className="overflow-x-auto">
          <table className="report-table min-w-[1200px]">
            ...
            <tbody>{visibleJobs.map((job) => <tr key={job.job_id}>
              ...
              <td>{new Date(job.created_at).toLocaleString()}<div className="mt-1 text-xs text-muted">更新：{new Date(job.updated_at).toLocaleString()}</div></td>
              <td><Link className="button button-secondary !px-3 !py-2" to={`/admin/jobs/${job.job_id}`}>详情<ExternalLink className="h-3.5 w-3.5" /></Link>{job.report_id && <Link className="mt-2 block text-center text-xs text-accent-strong hover:underline" to={`/admin/reports/${job.report_id}`}>报告</Link>}</td>
            </tr>)}</tbody>
          </table>
          {!loading && visibleJobs.length === 0 && <div className="p-10 text-center text-sm text-muted">当前页没有匹配任务</div>}
        </div>
      </section>
    </>
  );
